...
create_clock -name clock_50MHz -period 20 [get_pins u_pads_in/u_clk/C]

# TODO: Specify the uncertainty on the clock network (setup: 0.1 Tclk,
# hold: 4-5x or T_hold_dff, e.g. 50ps)
set_clock_uncertainty -setup 2 [get_clocks clock_50MHz]
set_clock_uncertainty -hold 0.05 [get_clocks clock_50MHz]

# TODO: Specify the clock transition (slew) on the specified clock (100 ps for TSMC 40n)
set_clock_transition 0.1 [get_clocks clock_50MHz]
...
set_input_delay 5 -clock [get_clocks clock_50MHz] [all_inputs]

# TODO: Constrain output ports within the design relative to a clock edge (try 1/4 of Tclk)
set_output_delay 5 -clock [get_clocks clock_50MHz] [all_outputs]

# TODO: Constrain the output rise time to 1/5 of the clock period
set_max_transition -rise 4 [get_clocks clock_50MHz]
